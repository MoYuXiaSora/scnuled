...
//接收缓存区 	
u8 RS485_RX_BUF[530];  	//接收缓冲,最大64个字节.
//接收到的数据长度
u8 RS485_RX_CNT=0;
u8 RXDData [530];
u8 DMXSignalFlag;
...
//--------------------------------????---------------------------------------------       
      ...
      // Disable the USART2 Transmit interrupt
				USART_ITConfig(USART2, USART_IT_TXE, DISABLE);         //??? ?????????? OK
}
}  											 
#endif										 
//初始化IO 串口2
...
{  
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;	//复用推挽
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; //485?
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);  
	...
	USART_InitStructure.USART_WordLength = USART_WordLength_9b;//9位数据长度
